...
//        String gensalt = BCrypt.gensalt();
    ...
    public boolean login(Admin admin) {
        // 需求，当系统关系用户需要管理后台时，需要先输入用户名和密码进行登录，才能进入管理后台
        // 1、用户发送请求。输入用户名和密码
        Admin admin2 = new Admin();
        admin2.setLoginName(admin.getLoginName());
        admin2.setStatus("1");
        Admin admin3 = adminMapper.selectByLoginName(admin2.getLoginName());
        ...
        //2、后台管理微服务conroller接收参数，验证用户名和密码是否正确，如果正确则返回用户登录成功结果
    }
